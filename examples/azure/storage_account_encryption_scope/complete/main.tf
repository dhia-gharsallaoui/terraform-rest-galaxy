...
# Two encryption scopes: one platform-managed, one customer-managed with
...
module "root" {
  ...
  azure_storage_account_encryption_scopes = {
    ...
    cmk = {
      ...
      resource_group_name               = var.resource_group_name
      account_name                      = var.account_name
      encryption_scope_name             = var.encryption_scope_name
      encryption_source                 = "Microsoft.KeyVault"
      key_vault_key_uri                 = var.key_vault_key_uri
